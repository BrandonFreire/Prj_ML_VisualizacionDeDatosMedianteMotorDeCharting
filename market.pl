...
printf
"  ZigZag externo: %d pivotes / %d segmentos  |  interno: %d pivotes / %d segmentos\n",
...
printf
  "  Internal ZigZag: %d pivotes / %d segmentos  |  ZonaInterna: %d niveles\n",
...
printf
  "  SuperTrend: %d  HalfTrend: %d  RangeFilter: %d  Supply: %d  Demand: %d\n",
...
my (
    $is_overlay_key_enabled,    $paint_overlay_categories,
    $update_overlay_menu_state, $add_overlay_separator,
    $add_overlay_toggle,        $add_overlay_action,
    $build_overlay_detail,      $show_overlay_group,
    $hide_overlay_menu,         $show_overlay_menu,
    $toggle_overlay_menu,
);
...
$build_overlay_detail = sub {
    ...
    if ( $group eq 'smc' ) {
        ...
        $add_overlay_action->(
            "\x{1F4C8} Dibujar Canal de Regresi\x{f3}n",
            sub {
                $hide_overlay_menu->() if defined $hide_overlay_menu;
                $mw->after(
                    50,
                    sub {
                        $engine->start_regression_channel_selection()
                          if defined $engine;
                    }
                );
            }
        ...
        $add_overlay_separator->();
        $add_overlay_action->(
            'Quitar Canal de Regresi' . "\x{f3}n",
            sub {
        ...
        $add_overlay_action->(
            ...
            sub {
                ...
                $mw->after(
                    50,
                    sub {
                        $engine->start_manual_fibonacci_selection()
                          if defined $engine;
                    }
                );
            }
...
sub _update_replay_ui {
    ...
    if ( $state eq 'exited' ) {
        ...
        $play_btn->configure( -text => '>', -fg => '#b2b5be' )
          if defined $play_btn;
    }
        ...
        $play_btn->configure( -text => '>', -fg => '#26a69a' )
          if defined $play_btn;
    }
    elsif ( $state eq 'playing' ) {
        $play_btn->configure( -text => '| |', -fg => '#f6c90e' )
          if defined $play_btn;
    }
...
$engine->add_overlay(
    Market::Overlays::ZigZagDirection->new(
        indicator  => $zz_ind,
...
$engine->add_overlay(
    Market::Overlays::SMC_Structures->new(
        indicator        => $smc_ind,
...
$engine->add_overlay(
    Market::Overlays::Liquidity->new(
        indicator  => $lq_ind,
        visibility => \%overlay_visibility,
    )
);
$engine->add_overlay(
    Market::Overlays::Strategy_Builder->new(
        indicator  => $strategy_ind,
        visibility => \%overlay_visibility,
    )
);
$engine->add_overlay(
    Market::Overlays::VolumeProfile->new(
        indicator  => $vp_ind,
        visibility => \%overlay_visibility,
    )
);
$engine->add_overlay(
    Market::Overlays::AnchoredVWAP->new(
        indicator  => $vwap_ind,
        visibility => \%overlay_visibility,
    )
);
...
$mw->bind( '<ButtonRelease-1>',
    [ sub { $engine->drag_end( $_[1], $_[2] ) }, Ev('X'), Ev('Y') ] );
$mw->bind( '<B1-Motion>',
